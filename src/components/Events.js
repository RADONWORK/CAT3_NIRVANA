...
const Events = () => {
  ...
  const DEFAULT_REGISTRATION_LINK = "https://payments.billdesk.com/bdcollect/bd/rnsiotec/6052"
  ...
  // Flatten in case events are grouped; filter by category only if present
  const allEvents = Array.isArray(events[0]) ? events.flat() : events;
  const filteredEvents = activeCategory === 'all'
    ? allEvents
    : allEvents.filter(event => event.category === activeCategory);

  // Group events by round property
  const round1Events = filteredEvents.filter(event => event.round === 1);
  const round2Events = filteredEvents.filter(event => event.round === 2);
  ...
  // Removed difficulty color logic since cards now show title, description, time, and venue only
  ...
  return (
    <section id="events" className="events section" ref={sectionRef}>
      <div className="container">
        ...
        {round1Events.length > 0 && (
          <div className="events-round">
            <h3 className="round-title fade-in">Round 1</h3>
            <div className="events-grid">
              {round1Events.map((event, index) => (
                <div
                  key={event.id || `${event.event_name}-r1-${index}`}
                  className={`event-card card fade-in`}
                ...
                >
                  ...
                  <button className="btn btn-primary event-register" onClick={() => handleOpenModal(event)}>
                    Register Now
                  </button>
                </div>
              ))}
        ...
        {/* {round2Events.length > 0 && (
          <div className="events-round">
            <h3 className="round-title fade-in">Round 2</h3>
            <div className="events-grid">
              {round2Events.map((event, index) => (
                <div
                  key={event.id || `${event.event_name}-r2-${index}`}
                  className={`event-card card fade-in`}
                  style={{ animationDelay: `${index * 0.1}s` }}
                >
                  <div className="event-header">
          ...
          <div className="modal-overlay" onClick={handleCloseModal} role="dialog" aria-modal="true">
            <div className="modal-content" onClick={(e) => e.stopPropagation()}>
              <div className="modal-header">
                <h3 className="modal-title">{selectedEvent.event_name} - Rules</h3>
                <button className="modal-close" onClick={handleCloseModal} aria-label="Close modal">×</button>
              </div>
              <div className="modal-body">
                <p className="modal-rules-text">{selectedEvent.rules || 'Rules will be provided soon.'}</p>
              </div>
              <div className="modal-footer">
                <a
                  className="btn btn-primary"
                  href={selectedEvent.registration_link || DEFAULT_REGISTRATION_LINK || '#'}
                  target="_blank"
                  rel="noopener noreferrer"
                  onClick={(e) => {
                    if (!(selectedEvent.registration_link || DEFAULT_REGISTRATION_LINK)) {
                      e.preventDefault();
                    }
                  }}
                >
        ...
        {filteredEvents.length === 0 && (
          <div className="no-events fade-in">
            ...
            <p>Try selecting a different category to see more events.</p>
          </div>
        )}
      </div>
    </section>
